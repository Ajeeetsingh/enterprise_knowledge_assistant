...
# =========================================================================== #
# set_auto_generated_title                                                     #
# =========================================================================== #


class TestSetAutoGeneratedTitle:
    def test_sets_title_when_conversation_has_none(
        self, service: ConversationService, owner: User
    ) -> None:
        conv = service.create_conversation(owner)
        assert conv.title is None

        updated = service.set_auto_generated_title(owner, conv.id, "Commercial Paper Issuers")

        assert updated is not None
        assert updated.title == "Commercial Paper Issuers"

    def test_persists_to_database(self, service: ConversationService, owner: User) -> None:
        conv = service.create_conversation(owner)
        service.set_auto_generated_title(owner, conv.id, "Project Phoenix")
        fetched = service.get_conversation(owner, conv.id)
        assert fetched.title == "Project Phoenix"

    def test_no_op_when_title_already_set(
        self, service: ConversationService, owner: User
    ) -> None:
        conv = service.create_conversation(owner, title="Existing Title")

        result = service.set_auto_generated_title(owner, conv.id, "Auto Generated")

        assert result is None
        fetched = service.get_conversation(owner, conv.id)
        assert fetched.title == "Existing Title"

    def test_no_op_after_a_prior_successful_auto_title(
        self, service: ConversationService, owner: User
    ) -> None:
        conv = service.create_conversation(owner)
        service.set_auto_generated_title(owner, conv.id, "First Auto Title")

        second_result = service.set_auto_generated_title(owner, conv.id, "Second Auto Title")

        assert second_result is None
        fetched = service.get_conversation(owner, conv.id)
        assert fetched.title == "First Auto Title"

    def test_strips_whitespace(self, service: ConversationService, owner: User) -> None:
        conv = service.create_conversation(owner)
        updated = service.set_auto_generated_title(owner, conv.id, "  Padded Title  ")
        assert updated is not None
        assert updated.title == "Padded Title"

    def test_blank_generated_title_is_a_no_op(
        self, service: ConversationService, owner: User
    ) -> None:
        conv = service.create_conversation(owner)
        result = service.set_auto_generated_title(owner, conv.id, "   ")
        assert result is None
        fetched = service.get_conversation(owner, conv.id)
        assert fetched.title is None

    def test_not_found_raises(self, service: ConversationService, owner: User) -> None:
        with pytest.raises(ConversationNotFoundError):
            service.set_auto_generated_title(owner, uuid.uuid4(), "Some Title")
    ...
    def test_foreign_conversation_raises_access_denied(
        ...
        conv = service.create_conversation(owner)
        with pytest.raises(ConversationAccessDeniedError):
            service.set_auto_generated_title(other_user, conv.id, "Stolen Title")
