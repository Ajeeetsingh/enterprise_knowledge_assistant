"""Persist orchestration Shadow results."""
...
class OrchestrationJsonStore:
    def __init__(self, root: Path) -> None:
        self.root = Path(root)
        self.root.mkdir(parents=True, exist_ok=True)
        self.snapshot_path = self.root / "orchestration_results.json"
        self._results: list[dict[str, Any]] = []
        existing = self.load()
        if existing:
            self._results = list(existing.get("results") or [])

    def append(self, result: dict[str, Any], *, limit: int = 200) -> Path:
