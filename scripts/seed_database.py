...
def main(argv: list[str] | None = None) -> int:
    ...
    parser.add_argument("--all", action="store_true", help="Seed roles, admin, demo users, and demo data.")
    parser.add_argument(
        "--skip-chat",
        action="store_true",
    ...
    if args.all:
        steps = ["roles", "admin", "demo_users", "demo_data"]
    else:
        steps = []
        if args.roles:
            steps.append("roles")
        ...
        if not steps:
            parser.error("Specify --roles, --admin, --demo, or --all")

    from seeding import admin, demo_data, demo_users, roles
